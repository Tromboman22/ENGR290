...
int main(void)
{
    ...
    while (1) {
        ...
        if (counter == 10 && perpendicular == 0 && control_fans(&hvc.us_sensor)){ // leftmost condition 1st in C...
            ...
            set_thrust(hvc.us_sensor.thrust_pwm * 0.9); // checkpoint turning power
            perpendicular--;
        } else {
            uart_puts("distance: ");
            uart_putfloat(hvc.us_sensor.distance, 1);
...
{ // function that will use distance to linearly increase brightness from 0-255
    ...
    sensor->thrust_pwm = 0.9 * sensor->max_fan_speed * // linearization over distance range
                     (sensor->distance - sensor->min_distance) /
                     (sensor->max_distance - sensor->min_distance); // basically find the fraction of max that you're at

    // lift pwm
    sensor->lift_pwm = sensor->thrust_pwm;
